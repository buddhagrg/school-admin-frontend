...
import { AcUnitOutlined, ExpandLess, ExpandMore } from '@mui/icons-material';
import {
  Box,
  Collapse,
  Divider,
...
import { getAppBase, getUserMenus } from '@/features/auth/auth-slice';
              ...
              return (
                <Box key={name}>
                  <ListItemButton onClick={() => handleNavigationClick(name)}>
                    ...
                    {openNavMenu === name ? <ExpandLess /> : <ExpandMore />}
                  </ListItemButton>
                  <Collapse
                    in={openNavMenu === name}
                    timeout='auto'
