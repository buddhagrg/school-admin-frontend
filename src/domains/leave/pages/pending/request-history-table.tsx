...
export const RequestHistoryTable = ({
  ...
  requests: MyLeaveRequestDetail[] | undefined;
}) => {
  const [updateLeaveRequest, { isLoading: isUpdatingLeaveRequest }] =
    useUpdateLeaveRequestMutation();
  const [deleteLeaveRequest, { isLoading: isDeletingLeaveRequest }] =
    useDeleteLeaveRequestMutation();
  ...
  const [selectedLeaveRequest, setSelectedLeaveRequest] =
    React.useState<MyLeaveRequestDetail | null>(null);
        ...
        {requests.map((request) => {
          ...
          return (
            <TableRow hover key={request.id}>
              <TableCell>
                ...
                <Menu open={isMenuOpen} anchorEl={anchorEl} onClose={handleMenuClose}>
                  {menusOptions.map((menu) => {
                    return (
                      <MenuItem key={menu} onClick={() => handleOptionClick(menu, request)}>
                        {menu}
                      </MenuItem>
                    );
                  })}
              ...
              <TableCell>
                {getFormattedDate(request.from, DATE_FORMAT)}
                {`—`}
                {getFormattedDate(request.to, DATE_FORMAT)} ({request.days} Days)
              </TableCell>
