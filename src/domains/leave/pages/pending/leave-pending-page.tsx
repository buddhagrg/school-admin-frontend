...
export const PendingRequest = () => {
            ...
            <TableRow hover key={leave.id}>
              <TableCell>
                ...
                <Menu open={isMenuOpen} anchorEl={anchorEl} onClose={handleMenuClose}>
                  <MenuItem
                    onClick={() =>
                      handleOptionClick('Approve', 2, leave.id, leave.policy, leave.user)
                    }
                  >
                    Approve
                  </MenuItem>
                  <MenuItem
                    onClick={() =>
                      handleOptionClick('Cancel', 3, leave.id, leave.policy, leave.user)
                    }
                  >
                    Cancel
                  </MenuItem>
              ...
              <TableCell>
                {getFormattedDate(leave.from, DATE_FORMAT)}
                {`—`}
                {getFormattedDate(leave.to, DATE_FORMAT)} ({leave.days} days)
              </TableCell>
  ...
  return (
    <Box>
      ...
      >
        <Typography sx={{ my: 2 }}>
          Are you sure you want to {modal.title} <strong>{modal.leaveType}</strong> request of{' '}
