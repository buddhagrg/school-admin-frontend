...
import { leaveDefineReducer, LeaveDefineState } from "../reducer/leave-define-reducer";
...
export const Overview = ({ leavePolicies }: { leavePolicies: LeavePolicy[] | undefined }) => {
    ...
    const handleClick = (event: React.MouseEvent<HTMLElement>, policyId: number, policyName: string, isPolicyActive: boolean) => {
        dispatch({
            type: "SET_MENU_CLICK",
            payload: {
    ...
    return (
        <>
            <Stack spacing={1} direction="row" sx={{ display: "flex", mt: 2 }}>
                <Typography component='div' sx={{ fontSize: "18px", fontWeight: 500 }}>Overview</Typography>
                <Button size='small' variant='outlined' startIcon={<AddCircle />} onClick={handleAddModalPolicy}>
                    Add New Policy
                </Button>
            </Stack>
            <Divider sx={{ my: "10px" }} />
                                    ...
                                    <Box sx={{ display: "flex", alignItems: "center" }}>
                                        ...
                                        <div style={{ marginLeft: "auto" }}>
                                            ...
                                            <Menu
                                                ...
                                                keepMounted
                                            >
                                                <MenuItem disabled={!state.isPolicyActive} onClick={handleEditModalPolicy}>Edit Policy</MenuItem>
                                                <MenuItem disabled={state.isPolicyActive} onClick={() => handlePolicyStatus(true)}>Enable Policy</MenuItem>
                                                <MenuItem disabled={!state.isPolicyActive} onClick={() => handlePolicyStatus(false)}>Disable Policy</MenuItem>
                                                <MenuItem disabled={!state.isPolicyActive} onClick={handleAddPeopleModal}>Add People</MenuItem>
                                            </Menu>
                                        </div>
                                    </Box>
