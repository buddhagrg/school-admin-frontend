...
export const leaveApi = api.injectEndpoints({
    endpoints: (builder) => ({
        getMyLeaveHistory: builder.query<LeaveRequestHistory, void>({
            query: () => `/leave/request`,
            providesTags: (result) => result?.leaveHistory.map(({ id }) => {
        ...
        getLeavePending: builder.query<PendingLeaveRequestHistory, void>({
            query: () => `/leave/pending`,
            providesTags: (result) => result?.pendingLeaves.map(({ id }) => {
        ...
        getLeavePolicies: builder.query<LeavePolicyData, void>({
            ...
            providesTags: (result) => result?.leavePolicies.map(({ id }) => {
        ...
        addUserToPolicy: builder.mutation<{ message: string }, AddUserToPolicy>({
            ...
            invalidatesTags: (result) => result
                ? [Tag.LEAVE_POLICY_USERS, Tag.LEAVE_ELIGIBLE_USERS, Tag.LEAVE_POLICIES, Tag.MY_LEAVE_POLICIES]
                : []
        }),
