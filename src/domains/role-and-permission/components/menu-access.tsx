...
import { Accordion, AccordionDetails, AccordionSummary, Box, Checkbox, FormControlLabel, Grid, List, ListItem } from "@mui/material";
...
export const MenuAccess: React.FC<MenuAccessProps> = ({
    ...
    const [updatePermissions, { isLoading: isUpdatingPermissions }] = useUpdateRolePermissionMutation();

    const togglePermission = (isChecked: boolean, currentRoleMenus: Permission[], menuId: number, subMenuId: number | null): Permission[] => {
    ...
    const handlePermissionToggle = (event: React.ChangeEvent<HTMLInputElement>, menuId: number, subMenuId: number | null) => {
        dispatch({
    ...
    const handleSave = async (event: React.MouseEvent<HTMLElement>) => {
        ...
        try {
            const ids = currentRoleMenus.flatMap(menu => {
                const subMenuIds = menu?.subMenus
                ...
                currentRoleMenus.map(({ id: menuId, name, subMenus, isPermissionAvailable }) => (
                    <Accordion key={menuId}>
                        ...
                        <AccordionDetails>
                            <Grid container>
                                {
                                    subMenus && subMenus.map(({ id: subMenuId, name, isPermissionAvailable }) => (
                                            ...
                                            <List>
                                                <ListItem>
                                                    <FormControlLabel
                                                        control={
                                                            <Checkbox
                                                                onChange={(event) => handlePermissionToggle(event, menuId, subMenuId)}
                                                                checked={isPermissionAvailable}
                                                                icon={<RadioButtonUnchecked />}
                                                                checkedIcon={<CheckCircleOutlined />}
                                                            />
