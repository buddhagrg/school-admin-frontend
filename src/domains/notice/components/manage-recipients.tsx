...
export const ManageNoticeRecipients: React.FC<ManageNoticeRecipientsProps> = ({
    ...
    const [updateRecipient, { isLoading: isUpdatingNoticeRecipient }] = useUpdateNoticeRecipientMutation();
    ...
    return (
        <Box component={Paper} sx={{ p: 2 }}>
            <Typography variant="subtitle1" sx={{ mb: 3 }}> {operation} Notice Recipient</Typography>
            < form onSubmit={handleSubmit(handleSave)} >
                <FormControl fullWidth sx={{ mt: 2 }} size="small" error={Boolean(errors.roleId)}>
                    <InputLabel id="role-for-dropdown" shrink>Roles</InputLabel>
                    <Controller
                        name="roleId"
                        control={control}
