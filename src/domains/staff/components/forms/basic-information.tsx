...
import { Box, FormControl, FormHelperText, Grid, InputAdornment, InputLabel, MenuItem, Select, Stack, TextField, Typography } from "@mui/material";
...
export const BasicInformation = () => {
    ...
    const { register, control, formState: { errors } } = useFormContext<StaffFormProps>();
    ...
    return (
        <>
            ...
            <Stack sx={{ my: 2 }} spacing={2}>
                        ...
                        render={({ field: { onChange, value }, fieldState: { error } }) => (
                            <>
                                ...
                                >
                                    {
                                        genders.map(gender =>
                                            <MenuItem value={gender.id} key={gender.id}>{gender.name}</MenuItem>
                                        )
                                    }
                                </Select>
                                <FormHelperText>{error?.message}</FormHelperText>
                ...
                <FormControl size="small" sx={{ width: "150px" }}>
                    <InputLabel id="maritalStatus" shrink>Mairtal Status</InputLabel>
                    <Controller
                        name="maritalStatus"
                        control={control}
                        render={({ field: { onChange, value }, fieldState: { error } }) => (
