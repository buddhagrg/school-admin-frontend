...
export const useHandleMenuAction = () => {
    ...
    const handleAction = async (menuItemValue: string, selectedId: number) => {
        const actionHandlers: { [key in keyof typeof menuItemTexts]: () => Promise<any> } = {
            ...
            "RESEND_VERIFICATION_EMAIL_TO_USER": () => resendVerificationEmail({ userId: selectedId }).unwrap(),
            ...
            "APPROVE_NOTICE": () => handleNoticeStatus({ id: selectedId, status: 5 }),
            "REJECT_NOTICE": () => handleNoticeStatus({ id: selectedId, status: 4 }),
            "DELETE_NOTICE": () => handleNoticeStatus({ id: selectedId, status: 6 }),
            "DELETE_NOTICE_BY_SELF": () => handleNoticeStatus({ id: selectedId, status: 3 }),
        };
